...
export function createMediaRecorder(stream: MediaStream, audioEnabled: boolean, videoFormat: string, onFinished: () => void) {
	const options = {
...
async function stopRecording(format: string, callback: () => void): Promise<void> {
	console.info('Renderer process: Recording stopped');
	deleteInterval();
