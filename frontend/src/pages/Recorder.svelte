		...
		{#if processReady}
			...
			{#if !recording}
				<div class={`${!panelOpened ? 'pointer-events-none ' : ''}flex items-center relative`}>
					...
					<!-- <button class={`${panelOpened ? 'cursor-pointer' : 'cursor-default'} absolute place-items-center w-3 h-3 -translate-x-1/2 left-1/2`} on:click={openSettings} disabled={!inputSources.length} aria-label="Options"> -->
					<!-- </button> -->
				</div>
			{:else}
